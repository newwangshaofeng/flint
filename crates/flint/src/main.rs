...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Flint failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.flint.Oops";
            proxy
                .add_notification(
                    notification_id,
...
fn main() {
    ...
    // `flint --crash-handler` Makes flint operate in minidump crash handler mode
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) =
            etw_tracing::record_etw_trace(flint_pid, &output_path, etw_socket.as_str())
        {
    ...
    // `flint --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    let failed_single_instance_check = if *flint_env_vars::ZED_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::flint::listen_for_cli_connections(open_listener.clone()).is_err()
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use flint::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("flint is already running");
        return;
    }
        ...
        Some(
            app.background_executor().spawn(crashes::init(
                InitCrashHandler {
                    ...
                    flint_version: semver::Version::new(
                        app_version.major,
                        app_version.minor,
                        app_version.patch,
                    )
                    .to_string(),
                    binary: "flint".to_string(),
                ...
                |pid| paths::temp_dir().join(format!("flint-crash-handler-{pid}")),
        ...
        match kind {
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("flint://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                if let Some(setting_path) = setting_path {
                    log::warn!("flint://settings/{setting_path} opens the settings file");
                }
                cx.spawn(async move |cx| {
                    let workspace =
                        workspace::get_any_active_multi_workspace(app_state, cx.clone()).await?;
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(Box::new(flint_actions::OpenSettingsFile), cx)
                    })
                })
                .detach_and_log_err(cx);
            }
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = flint::open_options_for_request(
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Flint would exit silently.
...
#[derive(Parser, Debug)]
#[command(name = "flint", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `flint://` scheme.
    ...
    /// On macOS, the default is `~/Library/Application Support/Flint`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/flint`.
    /// On Windows, the default is `%LOCALAPPDATA%\Flint`.
    ...
    /// Instructs flint to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Flint from starting, so you can't run `flint: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having Flint run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run flint in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// The PID of the Flint process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true, allow_hyphen_values = true)]
    etw_flint_pid: Option<i64>,
    ...
    /// Unix socket path for IPC with the parent Flint process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
